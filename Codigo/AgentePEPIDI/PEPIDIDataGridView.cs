...

{
    ...
    /// <summary>
    /// DataGridView personalizado com visual de cartões arredondados, badges coloridos
    /// por departamento/estado e coluna de ação opcional ("⋮"). O desenho é feito via
    /// GDI+ nos eventos RowPrePaint e CellPainting; as barras de scroll nativas ficam
    /// ocultas e o scroll é gerido manualmente por FirstDisplayedScrollingRowIndex.
    /// A cor do badge pode ser dinâmica: se BadgeColorColumnName estiver preenchida,
    /// o texto de contraste (preto ou branco) é calculado automaticamente a partir da
    /// luminância da cor de fundo. Todas as propriedades de estilo surgem na categoria
    /// "PEPIDI" no Designer do Visual Studio.
    /// </summary>
...

        {
...

            {
                ...
                corParaPintar = Color.FromArgb(
                    ...
                    Math.Max(0, corParaPintar.R - 20),

                    Math.Max(0, corParaPintar.G - 20),

                    Math.Max(0, corParaPintar.B - 20)
